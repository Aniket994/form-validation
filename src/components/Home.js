...
import { Country, State, City } from 'country-state-city'; //Country.getAllCountries() ; State.getAllStates();
...
const Home = () => {
    ...
    const optionCountry = [<option value="" key="" disabled>select country</option>, Country.getAllCountries().map((value,index) => <option value={value.name} key={index}>{value.name}</option>)]
    const optionState = [<option value="" key='' disabled>select state</option>, State.getAllStates().map((value,index) => <option value={value.name} key={index}>{value.name}</option>)]

    return (
        <>
            <div className="container">
                <div className="row">
                    <div className="col-lg-12 text-danger text-center mt-3 mb-3 header">
                        <h1>Form validation</h1>
                    </div>
                </div>
                <div className="row" >
                    <div className="col-lg-6 ">
                        <form className="border rounded-3 p-3 mb-3">
                            ...
                            <div className="d-flex mb-3 justify-content-between">
                                <div className="sc">
                                    ...
                                    <select className="form-select" value={countrys} onChange={obj => updateCountry(obj.target.value)}>
                                        {optionCountry}
                                    </select>
                                ...
                                <div className=" sc">
                                    ...
                                    <select className="form-select" value={states} onChange={obj => updateState(obj.target.value)}>
                                        {optionState}
                                    </select>
                                </div>
                            </div>
                    ...
                    <div className="col-lg-3">
                        {
                            user.map((user) => (
                                <UserDetails edit={edit} deleteItem={deleteItem} key={user.id} user={user} />
                            ))
                        }
                    </div>
                    <div className="col-lg-3">
